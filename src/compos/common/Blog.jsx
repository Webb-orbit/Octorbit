    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                if (blog) {
                    setblogdata(blog)
                    // contentref.current.innerHTML = plaintohtml(blog.content)
                }
            } catch (error) {
                console.log(error);
            }
        ...
        <div className='  '>
            <div className='py-5'>
                <div className=' mulish selection:text-black selection:bg-white'>
                    ...
                    <p className=' w-[80%] text-[0.9rem] text-neutral-200 font-medium'>{blogdata?.description}</p>
                </div>
                <div className=' mulish flex items-center py-8 justify-between selection:text-black selection:bg-white'>
                    ...
                    <div className='flex gap-3 items-center text-[0.9rem]'>
                        <button className='material-symbols-outlined  text-[1.4rem]'>favorite</button>
                        <button className='material-symbols-outlined  text-[1.4rem]'>share</button>
                    </div>
                </div>
            </div>
            <TracingBeam>
                <div > {parse(plaintohtml(blogdata.content))}</div>
            </TracingBeam>
        </div>
    ) : null
}
